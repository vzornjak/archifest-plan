...
- **Prostorija je jedinica izvještaja** (bez Polycam pretplate). RoomPlan-ova podjela se ne koristi kao struktura — cijeli sken dolazi kao jedan `rooms[0]` zapis, pa njegov `roomCount` znači "jedno snimanje", ne jedna prostorija. Alat sam rasterizira pod u finu mrežu, flood-fill pronalazi povezane prostorije (bez vanjske geometrijske biblioteke), klasificira ih glasanjem po tipu namještaja (Kuhinja/Kupaonica/Spavaća/Dnevni boravak/Praonica/Hodnik/Ormar) s geometrijskim vetoima, i tek onda **računa pod, strop i zidove po svakoj prostoriji zasebno**. Dijeli se samo tamo gdje stvarno postoje zidovi koji zatvaraju prostor — otvoreni plan ostaje jedna prostorija.
  - Površine soba normaliziraju se na poligon poda pa im je zbroj točno jednak ukupnom podu; mreža sama po sebi mjeri ~3% manje jer joj trake zidova pojedu ćelije
  - Dva sitna kontrolna retka ispod Pregleda: **pod** po sobi i zbroj naspram `meta.json`, te **zidovi** — `bruto − otvori` naspram metine neto vrijednosti i koliko je zida uopće pripisano sobama (hvata zid koji tiho ispadne iz podjele). Kontrola opsega je isprobana i odbačena: rasterska granica stepenasto ide oko kosih zidova pa sustavno odstupa 8–11% i vikala bi "greška" na svakom skenu
  - Dijeljeni zid ulazi punom površinom u OBJE sobe (svaka strana treba svoj premaz). Po prostoriji se prikazuju tri mjere: **bruto / neto / bojanje**
  - Tablica po sobi pokazuje **duljinu koju ta soba stvarno graniči** i prosječnu visinu na tom rasponu, pa se svaki redak množi u prikazanu površinu. Kosi zid nosi oznaku `kosina` i ispis računa `W·H − Σ(ΔW·ΔH)/2` s pravim brojevima — bez toga bruto koji nije `W × H` izgleda kao greška
- **Kosi zidovi** — površina se uzima iz `polygonCorners` (shoelace), ne iz `dimensions`; provjereno ručno na stvarnom skenu: pravokutnik `3.5991 × 2.4832` minus trokut krova `½ × 1.2249 × 0.8343` = **8.4263**. Podjela takvog zida između dvije sobe ide preko visine očitane po duljini (`wallHeightAt`), a ne po udjelu duljine — potonje je na stvarnom zabatu promašivalo do **20%** kod reza na četvrtini
- **Površina za bojanje** — zanatska konvencija, ne geometrija: otvor do **3 m²** se uopće ne odbija (vrijeme ušteđeno na rupi potroši se na špalete, ćoškove i zaštitu), a preko toga se odbija samo razlika (otvor od 4 m² odbija 1 m²). To je broj koji ide u troškovnik; neto ostaje geometrijski točan i služi kao kontrola prema `meta.json`. Na stvarnom skenu razlika je 11 m² (bojanje 74.11 vs neto 63.13)
...
- **Strop = pod prostorije + višak kosine.** Raniji model (`prosječni presjek × razmak kosih zidova`) morao je pogoditi dužinu za *cijeli* presjek, uključujući njegov ravni dio — a taj je već točno poznat iz poligona poda. Na stvarnom skenu je to puklo: 2.42 m stub-zid pokraj hodnika postavio je "zabate" 7.83 m jedan od drugoga u sobi dugoj 11.93 m, pa je ispalo 25.54 m² stropa nad podom od 33.34 m² — strop manji od sobe, koji se još i *smanjivao* kad se hodnik doda skenu (30.19 → 25.54).
  Rastavom na ravni dio i kosinu ostaje samo jedna nepoznanica, dužina kosine, i ona se mjeri: to je dužina najdulje strane s koljenastim zidom, prepoznatim po tome što mu visina odgovara koljenu iz presjeka (na stvarnom skenu 1.649 m naspram 2.483 m svih ostalih zidova). Koljenasti zidovi se grupiraju po stranama krova, pa se strana razbijena vratima opet spoji u jednu. Bez koljenastog zida pada se na `pod × (dužina presjeka / horizontalni raspon)`, što je ujedno i gornja granica.
  Testirani oblici presjeka, svaki na 0.0% greške: kosina po cijelom zidu (jednostrešni), dvije kosine u špic, ravni dio stropa s kosinom sa svake strane, te rubni slučajevi — koljenasti zid samo s jedne strane, koljenasti zidovi na različitim visinama, strana razbijena na više komada, i kosina koja ide do poda bez koljenastog zida.
